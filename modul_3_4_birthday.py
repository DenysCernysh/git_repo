...
def get_upcoming_birthdays(users):
    ...
    for user in users:
        birthday = datetime.strptime(user["birthday"], "%Y.%m.%d").date() #Перетворюємо рядкову дату народження в об'єкт datetime та отримуємо лише дату:
        birthday_this_year = birthday.replace(year=today.year) # визнач день народж в поточн році

        if birthday_this_year < today:
            birthday_this_year = birthday_this_year.replace(year=today.year + 1) #переносим на наступний рік якщо ДР минув
        ...
        if days_until_birthday <= 7:
            ...
            congratulation_date = today + timedelta(days=days_until_birthday) #Обчислюємо нову дату привітання та додаємо інформацію про користувача до списку upcoming_birthdays:
...
users = [
    ...
    {"name": "James Bond", "birthday": "1990.02.16"}
]
